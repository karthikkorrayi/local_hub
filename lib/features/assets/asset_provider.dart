...
final assetListProvider = FutureProvider<List<Asset>>((ref) async {
  ...
  if (folder != null) return db.assetDao.getAssetsByFolder(folder.id);
  return [];
});
...
final filteredAssetListProvider = Provider<List<Asset>>((ref) {
  ...
  return assetsAsync.when(
    data: (assets) => tag == null ? assets : assets.where((a) => a.tagList.contains(tag)).toList(),
    loading: () => [], error: (_, __) => [],
  );
});
...
final allTagsProvider = FutureProvider<List<String>>((ref) async {
  ...
  for (final raw in rawTags) {
    if (raw.isNotEmpty) tagSet.addAll(raw.split(',').map((t) => t.trim()).where((t) => t.isNotEmpty));
  }
...
class AssetActions {
  ...
  Future<void> addFolder(AssetFolder folder) async { await _folderDao.insertFolder(folder); _ref.invalidate(folderListProvider); }
  Future<void> updateFolder(AssetFolder folder) async { await _folderDao.updateFolder(folder); _ref.invalidate(folderListProvider); }
  Future<void> deleteFolder(AssetFolder folder) async {
    await _folderDao.deleteFolder(folder); _ref.invalidate(folderListProvider); _ref.invalidate(assetListProvider);
  }
  Future<void> addAsset(Asset asset) async { await _assetDao.insertAsset(asset); _ref.invalidate(assetListProvider); _ref.invalidate(allTagsProvider); }
  Future<void> updateAsset(Asset asset) async { await _assetDao.updateAsset(asset); _ref.invalidate(assetListProvider); _ref.invalidate(allTagsProvider); }
  Future<void> deleteAsset(Asset asset) async { await _assetDao.deleteAsset(asset); _ref.invalidate(assetListProvider); _ref.invalidate(allTagsProvider); }
}

...
class BackupState {
  ...
  const BackupState({
    this.isLoading = false, this.isSignedIn = false,
    this.lastResult, this.log = const [],
  });

  BackupState copyWith({bool? isLoading, bool? isSignedIn, BackupResult? lastResult, List<BackupLogEntry>? log}) =>
      BackupState(
...
class BackupNotifier extends StateNotifier<BackupState> {
  ...
  BackupNotifier(this._service) : super(const BackupState()) { _loadLog(); }
  ...
  Future<void> backup() async {
    ...
    final log = await _service.readLog();
    state = state.copyWith(isLoading: false, isSignedIn: _service.isSignedIn, lastResult: result, log: log);
  }
